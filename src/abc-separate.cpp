...
// Separate out one property in an Alembic file to its own flat binary file.
...
static BasePropertyReaderPtr find_property(CompoundPropertyReaderPtr prop,
                                           const std::string &name);
static BasePropertyReaderPtr find_property(BasePropertyReaderPtr prop,
                                           const std::string &name);
...
static std::pair<std::string, std::string> split(const std::string &name) {
  ...
  if (index == std::string::npos) {
    ...
    return std::make_pair(name.substr(0, index), name.substr(index + 1));
  }
}
...
static BasePropertyReaderPtr find_property(CompoundPropertyReaderPtr prop,
  ...
  if (!child) {
    std::cerr << name << " not found in property " << prop->getName()
              << " of object " << prop->getObject()->getName() << std::endl;
    return nullptr;
  }
...
static BasePropertyReaderPtr find_property(ObjectReaderPtr obj,
                                           const std::string &name) {
  auto parts = split(name);
  ...
  if (parts.first == "") {
    ...
    if (!prop) {
      std::cerr << name << " not found in object " << obj->getName()
                << std::endl;
      return nullptr;
    }
...
static bool write_property(ScalarPropertyReaderPtr prop, std::ofstream &out) {
  ...
  if (verbose) {
    std::cout << "writing " << prop->getNumSamples() << " samples, "
              << (size_t)dataType.getExtent() << " x "
              << PODName(dataType.getPod()) << " (" << bytesPerDatum
              << " bytes) per sample\n";
  }
...
static bool write_property(ArrayPropertyReaderPtr prop, std::ofstream &out) {
  ...
  std::cout << "samples are rank " << rank << " x " << dimension << " ("
...
static bool write_property(BasePropertyReaderPtr prop,
...
int main(int argc, const char *const *argv) {
  ...
  if (values.size() != 3) {
    std::cerr << "arguments: alembic-to-flat [-v|--verbose] input.abc "
                 "/object//property output.bin\n";
    std::cerr << "\n";
    std::cerr << "Reads the property (specified by path) from input.abc, dumps "
                 "the samples to the output.bin file\n";
    return 1;
  }
  ...
  BasePropertyReaderPtr property =
      find_property(archive.getTop().getPtr(), property_path);
  if (!property) {
    std::cerr << "property " << property_path << " not found in "
